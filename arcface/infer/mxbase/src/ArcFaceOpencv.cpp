...
APP_ERROR ArcFaceOpencv::Init(const InitParam &initParam) {
...
APP_ERROR ArcFaceOpencv::DeInit() {
  dvppWrapper_->DeInit();
  model_->DeInit();
...
APP_ERROR ArcFaceOpencv::ReadImage(const std::string &imgPath,
                                   cv::Mat *imageMat) {
  *imageMat = cv::imread(imgPath, cv::IMREAD_COLOR);
  return APP_ERR_OK;
}

APP_ERROR ArcFaceOpencv::ResizeImage(const cv::Mat &srcImageMat,
...
APP_ERROR ArcFaceOpencv::NormalizeImage(
    uint8_t (&imageArray)[IMG_H][IMG_W][IMG_C],
...
APP_ERROR ArcFaceOpencv::ArrayToTensorBase(float (&imageArray)[IMG_H][IMG_W][IMG_C],
...
APP_ERROR ArcFaceOpencv::Inference(
...
APP_ERROR ArcFaceOpencv::SaveResult(MxBase::TensorBase *tensor,
...
APP_ERROR ArcFaceOpencv::Process(const std::string &imgPath,
                                 const std::string &resultPath) {
  cv::Mat imageMat;
